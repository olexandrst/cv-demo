...
- `yolov8n.pt` (~6 МБ) — завантажиться при першому будь-якому режимі.
- Hard-hat модель (~6 МБ) — завантажиться при першому переключенні в **Засоби захисту**.
- Профілювання тягне три моделі в `./models/` при першому переключенні в **Профілювання**:
  - `yunet.onnx` (~230 КБ) — детектор облич;
  - `emotion-ferplus-8.onnx` (~35 МБ) — класифікатор емоцій;
  - `gender_net.caffemodel` (~45 МБ) — класифікатор статі.
